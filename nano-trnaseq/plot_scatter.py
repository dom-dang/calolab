...
def plot_scatter(outfn, tsv, samples=[]):
    ...
    for ax, y in zip(axes, cols[1:]):
        ...
        # Changed legend parameter to use one of the valid values
        sns.scatterplot(data=df2, x=x, y=y, ax=ax, hue="Aminoacid")
        ax.set_xscale('log'); ax.set_yscale('log')
        rho = df2[[x, y]].corr(method='spearman').to_numpy()[1,0]
        ax.set_title(title%rho)
        # Handle legend separately after creating plot
        if show_legend:
            ax.legend(ncol=2) 
        else:
            ax.get_legend().remove()  # Remove legend from this axis
