...
def get_ref2count(bam, mapq=1):
    ...
    for a in sam:
        ...
        # Check if unmapped using the flag (this is crucial)
        if a.flag & 4:  # Bit 4 indicates unmapped read
            ref2c["*"] += 1
            continue
        
        # Now safe to check reference name for mapped reads
        if a.mapq >= mapq:
            if a.is_reverse:
                ref2c["antisense"] += 1
            else:
                ref2c[a.reference_name] += 1
